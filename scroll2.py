...
if direction == "up": #cut the top off & move to bottom thus moving image up
    im_cropb = im.crop((0, span, 639, 479))
    im_cropt = im.crop((0, 0, 639, span+1))
...
if direction == "down": #cut the bottom off and move to top thus moving image down
    im_cropb = im.crop((0, 479-span, 639, 479))
    im_cropt = im.crop((0, 0, 639, 479-span))
    outimg=concat_v(im_cropb, im_cropt) #reverse order)


if direction == "left":
    im_cropL = im.crop((0, 0, span+1, 479))
    im_cropR = im.crop((span, 0, 639, 479))
    outimg=concat_h(im_cropR, im_cropL)

if direction == "right":
    im_cropR = im.crop((639-span, 0, 639, 479)) #20 pixels from the right)
    im_cropL = im.crop((0, 0, 639-span, 479))# the rest of the pixels
    outimg=concat_h(im_cropR, im_cropL)
